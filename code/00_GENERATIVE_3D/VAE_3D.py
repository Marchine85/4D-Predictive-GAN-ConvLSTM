...
TRAIN_VAL_RATIO     = 0.8               # Fraction of objects of full dataset to be used for training (use 0.0005 to just draw 1 sample for training)
...
VAE_ARCHITECTURE    = "Classic"          # Default: "PreRes" - "PreRes" or "Classic" Convolutions
###############################################################################
# Define LOSS    
LOSS                = 'ELBO'           # Default: KLDIV - Choose either KLDIV (Kullback-Leibler Divergence) or ELBO (optimize the single sample Monte Carlo estimate)
# If Loss is defined as KL-Divergence, set KLDIV Loss Weight
KL_DIV_WEIGHT       = 1.0               # Default: 1 - Weight factor relative to Object MSE Loss, to find balance during training
# Optimizer Settings
...
def ClassicEncoder():
    ...
    out_layer = Dense(Z_SIZE + Z_SIZE, activation=None)(x)
...
########## COMPILE NETWORKS ###################################################
###############################################################################
if VAE_ARCHITECTURE == "PreRes":
    decoder, encoder = PreResDecoder(), PreResEncoder()
elif VAE_ARCHITECTURE == "Classic":
...
def compute_loss(real_objects, real_labels, kl_div_weight=KL_DIV_WEIGHT):
    if LOSS == "ELBO":
        real_objects = tf.dtypes.cast(real_objects, tf.float32)
        mean, logvar = tf.split(encoder([real_objects, real_labels]), num_or_size_splits=2, axis=1) 
        z = reparameterize(mean, logvar)
        fake_objects = decoder([z, real_labels])
        cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_objects, labels=real_objects)
    ...
    if LOSS == "KLDIV":
        # Encoder Loss (KL-Divergence)
        real_objects = tf.dtypes.cast(real_objects, tf.float32)

        latent_code = encoder([real_objects, real_labels], training=True)
        mean, logvar = tf.split(latent_code, num_or_size_splits=2, axis=1)    
        # kl-divergence:
        enc_loss = tf.reduce_mean(-tf.reduce_sum(0.5 * (1 + logvar - mean ** 2 - tf.exp(logvar)), axis = 1), axis = 0)
        
        # Decoder Loss
        z = reparameterize(mean, logvar)
        fake_objects = decoder([z, real_labels], training=True)       
        ...
        # MSE for whole batch
        dec_loss = tf.reduce_mean(squared_sum)
        
        loss = dec_loss + kl_div_weight * enc_loss
        
        return (loss, dec_loss, kl_div_weight * enc_loss)
...
def generate_sample_objects(test_objects, epoch):
    ...
    for X, y in test_objects:
        ...
        if LOSS == "ELBO":
            sample_object = sigmoid(decoder([z, y], training=False))
        elif LOSS == "KLDIV":
            sample_object = decoder([z, y], training=False)

        object_out = np.squeeze(sample_object>0.5)

        #d.plotMeshFromVoxels(object_out, obj=model_directory+'sample_epoch_'+str(epoch)+'_label_'+str(y.numpy())+'_'+str(i+1))
...
def train_step(obj_batch, label_batch, LOSS):
    ...
    gradients_of_decoder = decoder_tape.gradient(loss[0], decoder.trainable_variables)
    gradients_of_encoder = encoder_tape.gradient(loss[0], encoder.trainable_variables)
...
if Restart_Training:
    ...
    model_directory = os.getcwd()+'/'+time_string+'_VAE_' + LOSS + '-loss' + '/'
...
for epoch in range(1+restart_epoch, 1+N_EPOCHS):
    ...
    for obj_batch, label_batch in objects:
        ...
        tloss = train_step(obj_batch, label_batch, LOSS)
        #Append arrays
        Train_loss.append(tloss[0].numpy())
        
        if LOSS == "ELBO":
            print("Epoch: [%2d/%2d] Batch: [%4d/%4d] elapsed_time: %.1f, dt_batch: %.2f, train_loss: %.4f" \
                % (epoch, N_EPOCHS, n_batch, training_set_size//BATCH_SIZE, time.time() - start_time, time.time() - batch_time, tloss[0]))
        elif LOSS == "KLDIV":
            print("Epoch: [%2d/%2d] Batch: [%4d/%4d] elapsed_time: %.1f, dt_batch: %.2f, total_loss: %.4f, Gen_loss: %.4f, KL-Div_loss: %.4f" \
                % (epoch, N_EPOCHS, n_batch, training_set_size//BATCH_SIZE, time.time() - start_time, time.time() - batch_time, tloss[0], tloss[1], tloss[2]))
    ...
    if (epoch % val_freq == 0) and val_freq != -1:
        ...
        for val_obj_batch, val_label_batch in val_objects:
            vloss = validation_step(val_obj_batch, val_label_batch, LOSS)
            Val_loss.append(vloss[0].numpy()) # Collect Val loss per Batch
    ...
    if epoch % save_freq == 0:
        ...
        render_graphs_vae(model_directory, epoch, train_loss, val_loss, epoch_arr, val_epoch_arr, Decoder_LR, Encoder_LR, LOSS) #this will only work after a 50 iterations to allow for proper averaging 
        # Save models
        decoder.save(model_directory+'trained_VAE_'+LOSS+'_epoch_'+str(epoch)+'.h5')
