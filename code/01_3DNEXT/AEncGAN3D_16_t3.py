...
T                   = 20                # Default: 80 - Even Number of Length of Time History to use, MUST BE EVENLY DIVISIBLE BY TINC, e.g. 80 means that 80 steps out of all 128 steps are used, where 24 steps at the beginning and 24 steps at the end of the time history are cropped.
TINC                = 1                 # Default: 1 - Allowed: (1, 2, 4, 8) - e.g.: 4 --> 80/4 = 20 Timepoints --> 19 PAIRS of T0->T1 --> TIME INCREMENT FOR EXTRACTING TIMESTEPS PER DATAPOINT -   (Check parse_tfrecords_4D_##_for_training for extraction details)
...
RESTART_FOLDER_NAME = "20240221_012216_aencgan3d_16_t3"
###############################################################################
# Output Settings
...
###############################################################################
# latent vector input
latent_vector = Input(shape=(Z_SIZE*3,))
#------------------------------------------------------------------------------
################### ENCODER ###################################################
#------------------------------------------------------------------------------
...
def encgen_loss(real_objects):
    ...
    real_objects_t2 = tf.dtypes.cast(real_objects[:,2], tf.float32)
    real_objects_t3 = tf.dtypes.cast(real_objects[:,3], tf.float32)
    
    # Encode
    latent_code_t0 = encoder(real_objects_t0)
    latent_code_t1 = encoder(real_objects_t1)
    latent_code_t2 = encoder(real_objects_t2)
    
    latent_code = tf.concat([latent_code_t0, latent_code_t1, latent_code_t2], axis=1)

    pred_objects_t3 = generator(latent_code, training=True)
    fake_logits = discriminator([pred_objects_t3, real_objects_t3], training=True)
    loss = wgan_gp_generator_loss(fake_logits)
    
    mse = tf.keras.metrics.mean_squared_error(real_objects_t3, pred_objects_t3)
...
def encdisc_loss(real_objects):
    ...
    pred_objects_t3 = generator(latent_code, training=True)
    fake_logits = discriminator([pred_objects_t3, real_objects_t3], training=True)
    real_logits = discriminator([real_objects_t3, real_objects_t3], training=True)
    D_loss, fake_loss, real_loss, gp, ep = wgan_gp_discriminator_loss(real_logits, fake_logits, real_objects_t3, pred_objects_t3, real_objects_t3, GP_WEIGHT, EPS_WEIGHT)
...
def val_encdisc_loss(val_real_objects):
    ...
    val_real_objects_t2 = tf.dtypes.cast(val_real_objects[:,2], tf.float32)
    val_real_objects_t3 = tf.dtypes.cast(val_real_objects[:,3], tf.float32)

    # Encode
    val_latent_code_t0 = encoder(val_real_objects_t0)
    val_latent_code_t1 = encoder(val_real_objects_t1)
    val_latent_code_t2 = encoder(val_real_objects_t2)
    
    val_latent_code = tf.concat([val_latent_code_t0, val_latent_code_t1, val_latent_code_t2], axis=1)

    val_pred_objects_t3 = generator(val_latent_code, training=False)
    val_fake_logits = discriminator([val_pred_objects_t3, val_real_objects_t3], training=False)
    val_real_logits = discriminator([val_real_objects_t3, val_real_objects_t3], training=False)
    D_loss, fake_loss, real_loss, gp, ep = wgan_gp_discriminator_loss(val_real_logits, val_fake_logits, val_real_objects_t3, val_pred_objects_t3, val_real_objects_t3, GP_WEIGHT, EPS_WEIGHT)
...
def MSE_reconstruction_loss(real_objects):    
    ...
    pred_objects_t3 = generator(latent_code, training=False)
...
def JACCARD_reconstruction_loss(val_obj_batch):    
    ...
    val_latent_code = tf.concat([val_latent_code_t0, val_latent_code_t1, val_latent_code_t2], axis=1)
    #pred_objects_t1 = generator([latent_code, real_labels, real_objects_t0], training=False)
    val_pred_objects_t3 = generator(val_latent_code, training=False)

    if NORMALIZE:
        real = np.asarray(val_real_objects_t3>0.0, bool)
        pred = np.asarray(val_pred_objects_t3>0.0, bool) 
    else:
        real = np.asarray(val_real_objects_t3>0.5, bool)
        pred = np.asarray(val_pred_objects_t3>0.5, bool) 
            
    j_and = tf.math.reduce_sum(np.double(np.bitwise_and(real, pred)), axis=[1,2,3,4])
    j_or = tf.math.reduce_sum(np.double(np.bitwise_or(real, pred)), axis=[1,2,3,4])
...
training_samples = training_set_size * (T//TINC-3) // BATCH_SIZE * BATCH_SIZE
...
for X in train_objects.take(N_SAMPLES):
    ...
    test_objects.append([Xt0_2[...,np.newaxis]]) # for validation of reconstruction during training, see generate_sample_objects():
    i+=1    
...
for X in val_objects.take(N_SAMPLES):
    ...
    valid_objects.append([Xt0_2[...,np.newaxis]]) # for validation of reconstruction during training, see generate_sample_objects():
    i+=1    
...
for epoch in range(1, N_EPOCHS+1):
    ...
    for obj_batch in train_objects:
        ...
        print("E: %3d/%5d | B: %3d/%3d | I: %5d | T: %5.1f | dt: %2.2f || LOSSES: D %.4f | G %.4f | MSE %.4f | JACCARD %.4f" \
            % (epoch, N_EPOCHS, n_batch, np.ceil(training_set_size*(T//TINC-3)//BATCH_SIZE), total_iterations, time.time() - start_time, time.time() - batch_time,  -D_loss[0], G_loss[0], G_loss[1], JACCARD_loss))
    ...
    n_objects = n_objects + np.ceil(training_set_size*(T//TINC-3)//BATCH_SIZE)*BATCH_SIZE
    print("\n--------------------------------------------------------------------")
