...
def read_tfrecord(example):
    ...
    for t in range(object4d.shape[0]):
        if t == object4d.shape[0] - 3:
            break
        rearranged_objs4d.append(object4d[t])
        rearranged_objs4d.append(object4d[t+1])
        rearranged_objs4d.append(object4d[t+2])
        rearranged_objs4d.append(object4d[t+3])

    object4d = tf.reshape(tf.stack(rearranged_objs4d), [T//TINC-3, 4, 16, 16, 16])
    
    #print("Time_Pairs_to_Train_per_Sample: ", object4d.shape[0])
    #object4d = tf.reshape(tf.stack(rearranged_objs4d), [127, 2, 16, 16, 16])
    if NORMALIZE:
...
def get_dataset(filenames: list, shuffle: bool, batch_size: int, t: int, tinc: int, normalize: bool):
    ...
    if shuffle:
        dataset = dataset.shuffle(len(filenames)*(T//TINC-3)//2) # ALL: len(filenames)*(T//TINC)
