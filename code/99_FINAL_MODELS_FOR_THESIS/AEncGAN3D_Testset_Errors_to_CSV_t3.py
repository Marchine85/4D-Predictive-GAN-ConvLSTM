...
def evaluate_predictions(objects, compare_to):
    ...
    for X in objects:
        ...
        X1 = tf.dtypes.cast(X[:,3], tf.float32)

        if compare_to==0: ref = X0
        elif compare_to==-1: ref = Xminus1 
        elif compare_to==-2: ref = Xminus2
        ...
        MSE_LOSSES_per_batch_NULL = MSE_reconstruction_loss(X1, ref).numpy()
        real = np.asarray(X1>0.5, bool)
        fake = np.asarray(ref>0.5, bool) 
        ...
        MSE_LOSSES_per_batch_INPUT = MSE_reconstruction_loss(ref, pred_object).numpy()
        real = np.asarray(ref>0.5, bool)
        fake = np.asarray(pred_object>0.5, bool) 
...
for COMPARE_TO in COMPARE_TO:
    MSE_LOSSES_all, MSE_LOSSES_all_NULL, MSE_LOSSES_all_INPUT, JACCARD_LOSSES_all, JACCARD_LOSSES_all_NULL, JACCARD_LOSSES_all_INPUT = evaluate_predictions(test_objects, COMPARE_TO)
    ...
    np.savetxt(f"evals/AEncGAN3D_TestSet_Errors_T{T}_TINC{TINC}_t3_toX{COMPARE_TO}.txt" , LOSSES, fmt="%20.5f, %20.5f, %20.5f, %20.5f, %20.5f, %20.5f", delimiter=',', header=header)
