...
T                   = 40                # Default: 80 - Even Number of Length of Time History to use, MUST BE EVENLY DIVISIBLE BY TINC, e.g. 80 means that 80 steps out of all 128 steps are used, where 24 steps at the beginning and 24 steps at the end of the time history are cropped.
TINC                = 2                 # Default: 1 - Allowed: (1, 2, 4, 8) - e.g.: 4 --> 80/4 = 20 Timepoints --> 19 PAIRS of T0->T1 --> TIME INCREMENT FOR EXTRACTING TIMESTEPS PER DATAPOINT -   (Check parse_tfrecords_4D_##_for_training for extraction details)
