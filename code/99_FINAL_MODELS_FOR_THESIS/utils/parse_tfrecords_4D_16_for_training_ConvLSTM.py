...
def read_tfrecord(example):
    ...
    x_object4d = object4d[0 : object4d.shape[0] - 1, :, :]
    y_object4d = object4d[1 : object4d.shape[0], :, :]
    
    if NORMALIZE:
        object4d = tf.cast(object4d, tf.float32) - 0.5
        
    return x_object4d, y_object4d #, tf.ones([object4d.shape[0]-1,4], tf.uint8)*labelObjs
...
def get_dataset(filenames: list, shuffle: bool, batch_size: int, t: int, tinc: int, normalize: bool):
    ...
    if shuffle:
        dataset = dataset.shuffle(len(filenames)*(T//TINC)//2) # ALL: len(filenames)*(T//TINC)
